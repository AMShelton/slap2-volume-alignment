...
ShiftMode = Literal["selected", "symmetric"]
...
<<<<<<< HEAD
    shift_mode:
        ...
        opposite row parity by ``-bidiphase / 2``. Symmetric mode preserves the
...
<<<<<<< HEAD
    def _shift_row_subset(row_slice: slice, row_phase: float) -> None:
        ...
        abs_row_phase = abs(row_phase)
        is_integer = float(row_phase).is_integer()
        if is_integer:
            p = int(row_phase)
            if p > 0:
                out[row_slice, p:] = original[row_slice, :-p]
                if fill_mode == "nearest":
                    out[row_slice, :p] = original[row_slice, :1]
                elif fill_mode == "zero":
                    out[row_slice, :p] = 0
        ...
        selected = original[row_slice].astype(np.float32, copy=False)
        ...
        out[row_slice] = shifted

        edge = int(np.ceil(abs_row_phase))
        if edge > 0:
            if fill_mode == "preserve":
                if row_phase > 0:
                    out[row_slice, :edge] = original[row_slice, :edge]
                else:
                    out[row_slice, -edge:] = original[row_slice, -edge:]
            elif fill_mode == "zero":
                if row_phase > 0:
                    out[row_slice, :edge] = 0
                else:
                    out[row_slice, -edge:] = 0
            elif fill_mode == "nearest":
                pass

    if shift_mode == "selected":
        _shift_row_subset(selected_rows, phase)
    else:
